...
z_pvalue = z_save = y_ess = y_pred_ess <- list()
...
for (s in 1:25) {
  ...
  y_pred_ess0 <- apply(out$y_pred_save, 1, coda::effectiveSize)
  y_pred_ess[[s]] <- y_pred_ess0
  y_ess[[s]] <- c(apply(out$y_save, 1, coda::effectiveSize), y_pred_ess0)
...
saveRDS(list(rmspe = rmspe,
             ...
             data0 = data0,                                                     # seed = 3
             out = out),
        paste0(path, "sim2/sim2b_bag_all.RDS"))
